...
const PostsTable = dynamic(() => import("@/components/PostsTable"), {
  ssr: false,
});
const UpdatePost = dynamic(() => import("@/components/UpdatePost"), {
  ssr: false,
});
  ...
  const handlePublish = useCallback(
    async (postId, isPublished) => {
      try {
        ...
        showAlert(
          "success",
          `Post ${isPublished ? "publié" : "mis en brouillon"} avec succès`
        );
        fetchPosts(1);
      } catch (error) {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-30 backdrop-blur-sm flex justify-center items-center">
          <div className="bg-white p-6 rounded-lg max-w-md w-full">
            ...
            <p className="text-sm text-gray-500 mb-4">
              Êtes-vous sûr de vouloir supprimer le(s) post(s) sélectionné(s) ?
